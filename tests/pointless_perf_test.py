...
def perf():
    items = [randint(1, 8) for _ in range(20_000_000)]

    start = perf_counter()
    calc_nsqrt_perf(items)
    end = perf_counter()
    total = end - start
    print('NSQRT')
    print(f'Total time = {total}\naverage time = {total / 1_000_000}')

    start = perf_counter()
    calc_sqrt_perf(items)
    end = perf_counter()
    total = end - start
    print('SQRT')
    print(f'Total time = {total}\naverage time = {total / 1_000_000}')
